...
LOG_FILE = SCRIPT_DIR / "log_headless.txt"
...
# In headless mode, prefer URL query-based page jumps by default
PAGE_JUMP_BY_QUERY = os.getenv("PAGE_JUMP_BY_QUERY", "1").lower() in {"1", "true", "yes"}

# Quiet window mode for headful runs (minimize/move off-screen)
...
with sync_playwright() as p:
    ...
    # Headless by default for this variant
    headless_mode = os.getenv("PLAYWRIGHT_HEADLESS", "1").lower() in {"1", "true", "yes"}
    print(f"[RUN] HEADLESS={headless_mode} BROWSER={browser_name}")
    ...
    if browser_name == "chromium":
        ...
        # Quiet (headful) mode: minimize and move off-screen on Windows
        if (not headless_mode) and PLAYWRIGHT_QUIET and os.name == "nt":
            launch_args += ["--start-minimized", "--window-position=-32000,-32000", "--mute-audio", "--disable-notifications"]
        # Taller viewport helps pagination appear in headless
