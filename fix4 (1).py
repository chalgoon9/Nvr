...
def product_list_crawl(context, df, read_excel_path, seen_urls):
    ...
    # Safe return to avoid NameError, even if lists stayed empty
    log(f"RETURN common_urls={len(common_urls)} different_urls={len(different_urls)}"); return common_urls, different_urls
...
def get_visible_page_numbers():
    """Return sorted unique page numbers currently rendered inside the pagination component."""
    container = find_pagination_container()
    if not container:
        return []
    numbers = []
...
def ensure_group_has_page(target_page, max_group_hops=30):
    """Navigate 10-by-10 groups until the group containing target_page is visible."""
    hops = 0
...
def click_pagination_control(direction):
    """Click 'next' or 'prev' inside pagination. Return True if click likely succeeded."""
