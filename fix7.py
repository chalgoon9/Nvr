# ==== EARLY DEBUG BOOTSTRAP (must appear before any usage) ====

# ==== SAFE TRACING WRAPPER (early) ====
def safe_start_tracing(context):
    try:
        # Prefer local start_tracing if already defined
        if 'start_tracing' in globals():
            try:
                safe_start_tracing(context)
                return
            except Exception as e:
                log(f"start_tracing() raised: {e}")
        # Fallback: direct tracing API
        try:
            context.tracing.start(screenshots=True, snapshots=True, sources=True)
            log("Tracing started via fallback.")
        except Exception as e:
            log(f"Fallback tracing start failed: {e}")
    except Exception as e:
        log(f"safe_start_tracing unexpected error: {e}")
# =======================================
import os, sys, time
from pathlib import Path

SCRIPT_DIR = Path(__file__).resolve().parent
DEBUG_DIR = os.getenv("DEBUG_DIR", str(SCRIPT_DIR / "debug"))
RUN_LOG_PATH = os.path.join(DEBUG_DIR, "run.log")
try:
    os.makedirs(DEBUG_DIR, exist_ok=True)
    open(RUN_LOG_PATH, "a", encoding="utf-8").close()
except Exception:
    pass
...
def stop_tracing(context, path=None):
    try:
        if path is None:
            path = os.path.join(DEBUG_DIR, "trace.zip")
...
try:
    os.makedirs(DEBUG_DIR, exist_ok=True)
    # touch the run.log immediately so it's visible even before first log()
    open(os.path.join(DEBUG_DIR, "run.log"), "a", encoding="utf-8").close()
except Exception:
    pass
...
def product_list_crawl(context, df, read_excel_path, seen_urls):
    # --- ensure return vars always exist ---
    common_urls = []
    different_urls = []
    # ---------------------------------------
    ...
    log(f"RETURN common_urls={len(common_urls)} different_urls={len(different_urls)}")
...
def go_to_page_number(target_page):
    ...
    if link:
        ...
        if num == target_page:
            print(f"[go_to_page_number] now at page {target_page}")
            return True
        after = get_first_list_href()
        if before and after and before != after:
            print(f"[go_to_page_number] list changed; assume reached {target_page}")
            return True
...
with sync_playwright() as p:
    ...
    elif browser_name == ox.launch(headless=headless_mode)
        context = browser.new_context()
    else:
