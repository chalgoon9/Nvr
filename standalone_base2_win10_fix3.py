...
def product_list_crawl(context, df, read_excel_path, seen_urls):
    ...
    def url_prefix(u):
        ...
        # --- build unique_urls from current product list (if not already defined) ---
        try:
            unique_urls  # type: ignore
        except NameError:
            unique_urls = []
            try:
                # try common list container selectors
                containers = page.query_selector_all("ul,div")
                # Prefer anchors under product list sections
                ...
                for a in candidates:
                    ...
                    if not href:
                        continue
                    # keep only smartstore product detail links
                    if "/products/" in href:
                        if href not in seen:
                            seen.add(href)
                            unique_urls.append(href)
                # fallback: if nothing found, at least avoid crashing
            except Exception:
                unique_urls = []
        # --- end build unique_urls ---
